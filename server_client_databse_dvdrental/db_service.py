...
class DBService:
    ...
    # Read
    def read(self):
        self.cursor.execute("SELECT * FROM actor ORDER BY actor_id asc")
        return self.cursor.fetchall()

    # Read by scope
    def read_by_id_scope(self, id_start, id_end):
        query = "SELECT * FROM actor WHERE actor_id >= %s AND actor_id <= %s ORDER BY actor_id ASC"
        self.cursor.execute(query, (id_start, id_end))
        return self.cursor.fetchall()
